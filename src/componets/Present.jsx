...
const Present = ({item, handleAddReserve}) => {

    const buttonType = () => {
        if (!item.isReserved) {
            return <button
                        onClick={() => handleAddReserve(item.id)}
                        className='w-full border-2 py-2 px-5 rounded-md border-green-500 text-green-500 mt-auto hover:bg-green-500 hover:text-white'>
                        Беру себе
                    </button>
        } else {
            return <button
    ...
    return (
        <div className='flex flex-col shadow-xl rounded-xl bg-white'>
            ...
            <div className='flex flex-col px-5 pb-3 grow'>
                ...
                {item.url && <a className='block text-xl text-green-300 underline hover:no-underline mb-3' href={item.url}>Ссылка на пример</a>}
                { buttonType() }
            </div>
        </div>
    );
